...
/*
 * Emscripten/Wasm has no real controlling terminal (the JVM runs inside a
 * browser sandbox), so no native pty is available.
 */
class JDKNativePty {

    static JnaNativePty current(TerminalProvider.Stream console) throws IOException {
        throw new UnsupportedOperationException("No pty on emscripten");
    }
