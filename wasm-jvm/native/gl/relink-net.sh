#!/usr/bin/env bash
# Build the networking JVM variant: build/.../web/jvmnet.js (PROXY_POSIX_SOCKETS). Compiles the GL JNI
...
emcc -c -O2 -DUSE_PROXY_SOCKETS "$JDK/wasm-jvm/native/gl/netstub.c" -I jdk/include -I jdk/include/emscripten -o glbin/netstub_net.o || { echo NETSTUB_FAIL; exit 1; }
emcc -c -O2 -pthread -matomics -mbulk-memory "$JDK/wasm-jvm/framework/kernel/wsps.c" -I jdk/include -I jdk/include/emscripten -o glbin/wsps.o || { echo WSPS_FAIL; exit 1; }
...
emcc -c -O2 -I jdk/include -I jdk/include/emscripten glbin/net_symtab.c -o glbin/net_symtab.o
echo "gl symtab: $N symbols (GLDemo natives: $(grep -c Java_GLDemo /tmp/gl_syms.txt))"
emcc web/launcher_web.c glbin/net_symtab.o awtobj/*.o fontobj/*.o glbin/wgl.o glbin/lwjgl.o glbin/netstub_net.o glbin/wsps.o $(ls glbin/gl_gen.o glbin/al_gen.o glbin/openal.o 2>/dev/null) \
  ...
  -sPROXY_POSIX_SOCKETS -lwebsocket.js \
  -Wl,--wrap=read -Wl,--wrap=write -Wl,--wrap=close -Wl,--wrap=readv -Wl,--wrap=writev \
  -sINITIAL_MEMORY=1610612736 -sSTACK_SIZE=8388608 -sWASM_BIGINT -sERROR_ON_UNDEFINED_SYMBOLS=0 -sEXIT_RUNTIME=1 \
  -lidbfs.js \
  -sEXPORT_NAME=createJVMNet -sMODULARIZE=1 -sFORCE_FILESYSTEM=1 \
  ...
  -o web/jvmnet.js 2>&1 | grep -iE "error:|duplicate symbol" | head
echo "LINKED jvmnet.js"
